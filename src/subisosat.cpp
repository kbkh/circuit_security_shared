    ...
    void *arg)
{
    *iso = false;
    ...
    for (unsigned int vid2 = 0; vid2 < igraph_vector_size(&map); vid2++) { // ... for every vertex in the map see if it can map to the vertex it is mapped to.
...
//    igraph_set_error_handler(igraph_error_handler_ignore);
     ...
     ******************************/
    // Check if vertices are indeed the same
    for (unsigned int vid2 = 0; vid2 < igraph_vcount(graph2); vid2++) {
        ...
        for (unsigned int vid1 = 0; vid1 < igraph_vcount(graph1); vid1++) {
            ...
            bool match(true);
            if (vertex_colour1 != NULL && vertex_colour2 != NULL)
                if (VECTOR(*vertex_colour1)[vid1] != VECTOR(*vertex_colour2)[vid2]) // check if same gate
                    match = false;

            if (node_compat_fn != NULL)
                if (!(*node_compat_fn)(graph1, graph2, vid1, vid2, arg)) // check if they have the same amount of i/o seperatly. (i=i; o=o) or 2 has less but not more. 2 is H and 1 is G
                    match = false;
